...
/*   Updated: 2024/12/10 15:32:37 by ompelman      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
{
	while (str[*i] && pf_strchr("#0- +", str[*i]))
		utils->flags |= get_flag(str[(*i)++]);
	while (str[*i] && pf_strchr("0123456789", str[*i]))
		utils->pad = utils->pad * 10 + str[(*i)++] - 48;
	if (pf_strchr("%0123456789", str[*i]))
	{
		utils->flags = reset;
		utils->flags |= percent;
		return (false);
	}
	if (!str[*i] || !pf_strchr("cspiduxX%", str[*i]))
		return (false);
	else if (utils->flags & zero || utils->flags & dash)
		if (!pf_strchr("0123456789", str[*i - 1]))
			return (false);
	return (true);
}
