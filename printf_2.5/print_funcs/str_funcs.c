...
/*   Updated: 2024/12/03 16:22:22 by ompelman      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static bool	left_padding(t_utils *utils, int slen);
static bool	right_padding(t_utils *utils, int slen);
...
bool	_char(t_utils *utils)
{
	...
	c = (unsigned char) va_arg(*(utils->arg), int);
	if (!c)
		return (NULL);
...
static bool	left_padding(t_utils *utils, int slen)
{
	if ((utils->flags & (zero | dash)) == zero && utils->pad >= slen)
	{
